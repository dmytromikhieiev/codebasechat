...
- **Явно названный файл больше не теряется в top-K** — ✅, см.
  `.claude/tasks/explicit-file-mention.md`. Вопрос "расскажи подробнее про
  docker-compose.prod.yml" отвечался обрывками из `DEPLOY.md` — другой файл
  упомянул "docker-compose" в прозе и обошёл сам YAML по реранк-скору,
  `RERANK_TOP_K=5` не гарантирует место для явно названного файла.
  Рассматривался regex по вопросу ("похоже на имя файла") — отклонён как
  ненадёжный на естественном языке; вместо этого
  `retrieval.find_mentioned_file_chunks` сверяет вопрос с уже известным
  списком проиндексированных путей репозитория (только реальные имена,
  без угадывания по паттерну), при коллизии одинаковых basename — предпочёт
  путь, чей полный путь тоже упомянут, иначе — наименее вложенный. Найденные
  чанки (до `MAX_PINNED_FILE_CHUNKS=5`) гарантированно попадают в контекст
  ответа в `api/routers/ask.py`, не участвуя в реранк-конкуренции; при их
  наличии `top_k` реранка для остальных кандидатов уменьшается, но не ниже
  1 — часть общего контекста сохраняется.
