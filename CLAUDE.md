...
- **Pluggable-провайдеры внешних API**: если у сервиса может быть несколько
  взаимозаменяемых бэкендов (сейчас — генерация ответа: OpenAI/Anthropic,
  `api/services/answer.py`), оформлять как `ABC` + конкретные реализации +
  фабрика, читающая выбор из переменных окружения (`*_PROVIDER`/`*_MODEL`),
  а не `if provider == "x"` россыпью по коду. Публичная функция-обёртка
  (`stream_answer`) сохраняет сигнатуру — вызывающий код (роутер) не знает
  о существовании выбора.
